...
class DPTHead(nn.Module):
    ...
    def forward(self, features, patch_h, patch_w, return_fused=False):
        ...
        assert len(features) == self.n_layers, \
            f"Expected {self.n_layers} features, got {len(features)}"

        expected_n = patch_h * patch_w
        actual_n = features[0].shape[1]
        assert actual_n == expected_n, \
            f"patch_h*patch_w={expected_n} but features[0] has N={actual_n}"

        out = []
        for i, x in enumerate(features):
            # (B, N, C) → (B, C, N) → (B, C, patch_h, patch_w)
...
class DPTSimpleHead(DPTHead):
    ...
    def __init__(self, embed_dim=768, features=256):
        super().__init__(embed_dim=embed_dim, features=features, n_layers=4)
    ...
    def forward(self, features, patch_h, patch_w):
        ...
        """
        return super().forward(features, patch_h, patch_w)
